...
$(document).ready(function() {
    ...
    function loadFaq(id) {
        ...
        $.get(`<?= site_url('faq/ajax') ?>/${id}`, function(res) {
            ...
            $('#faqList').html('<div class="text-danger text-center py-10">Ralat memuatkan data.</div>');
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post('<?= base_url('faq/save') ?>', result.value, function(res) {
                    ...
                    let errorMsg = (response && response.message) ? response.message : 'Gagal simpan (404/500). Semak Network Tab.';
