...
<style>
    ...
    /* 2. Hide Dashboard Header (From Main Layout) */
    ...
    /* 4. Fix Input Padding (Elak Bertindih dengan Icon) */
    .input-with-icon {
        padding-left: 3rem !important; /* 48px dari kiri */
        padding-right: 2.5rem !important; /* Ruang untuk arrow dropdown */
    }
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 mb-6">
        ...
        <div class="md:col-span-8 relative">
            ...
            <input type="text" id="searchDokumen" placeholder="Cari tajuk dokumen..." 
                   class="input-with-icon w-full bg-white border border-slate-200 p-3 rounded-xl focus:outline-none">
        </div>
    </div>
...
<div id="viewModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm hidden flex items-center justify-center z-50 p-4" style="z-index: 9999;">
    <div class="modal-container bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl animate-[slideUp_0.3s_ease-out]">
        <div class="bg-slate-50 p-5 flex justify-between items-center border-b">
            ...
            <button id="closeViewModal" class="text-slate-400 hover:text-slate-600 transition text-2xl border-0 bg-transparent"><i class="bi bi-x-circle-fill"></i></button>
        </div>
        <div id="dokumenDetails" class="p-8 max-h-96 overflow-y-auto"></div>
        <div class="p-6 bg-slate-50 flex justify-end">
            <button id="btnTutupModal" class="bg-white border px-6 py-2 rounded-xl font-bold hover:bg-gray-50">Tutup</button>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadData(page = 1) {
        ...
        try {
            ...
            if (result.data && result.data.length > 0) {
                // Kalau ada data, panggil function populate
                populateTable(result.data, result.pagination);
            } else {
                // --- FIX: KALAU TIADA DATA (0 REKOD) ---
                tbody.innerHTML = `
                    <tr>
                ...
                `;
                // Set text manual untuk 0 rekod supaya tak error
                document.getElementById('totalInfo').innerText = 'Menunjukkan 0 daripada 0 rekod';
                paginationContainer.innerHTML = '';
            }
        } catch (err) {
            console.error(err);
            tbody.innerHTML = '<tr><td colspan="6" class="text-center text-red-500 p-5">Ralat memuatkan data.</td></tr>';
        }
    }
    ...
    function populateTable(data, pagination) {
        ...
        // --- FIX: LOGIC KIRAAN REKOD (CONTOH: 1-5 DARIPADA 5) ---
        const start = (currentPage - 1) * pagination.limit + 1;
        const end = start + data.length - 1;
        
        // Update text info kat bawah table
        ...
        data.forEach((d, index) => {
            ...
            tr.innerHTML = `
                ...
                <td class="p-4">
                    <div class="flex items-center justify-between cursor-pointer group" onclick="showDokumenModal('${d.iddoc}')" title="Klik untuk lihat detail">
                        <div>
                ...
                <td class="p-4"><span class="text-xs bg-slate-100 text-slate-500 px-2 py-1 rounded font-bold uppercase">${d.mime ? d.mime.split('/')[1] : 'FILE'}</span></td>
                <td class="p-4 text-center"><span class="status-pill status-${statusLabel}">${statusLabel}</span></td>
                <td class="p-4 text-slate-500 text-sm"><div class="flex items-center gap-2"><i class="bi bi-clock-history"></i> ${formatDate(d.created_at)}</div></td>
                <td class="p-4">
                    <div class="flex justify-center gap-2">
                        ...
                        <button class="approveBtn btn-action bg-emerald-50 text-emerald-600 p-2 rounded-xl hover:bg-emerald-600 hover:text-white transition" data-id="${d.iddoc}"><i class="bi bi-check-lg pointer-events-none"></i></button>
                        <button class="rejectBtn btn-action bg-rose-50 text-rose-600 p-2 rounded-xl hover:bg-rose-600 hover:text-white transition" data-id="${d.iddoc}"><i class="bi bi-x-lg pointer-events-none"></i></button>
                    </div>
                </td>
            `;
    ...
    function renderPagination(totalPages) {
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            btn.textContent = i;
            btn.className = `w-10 h-10 rounded-xl font-bold transition ${i === currentPage ? 'bg-indigo-600 text-white' : 'bg-white text-slate-500 hover:bg-slate-100'}`;
    ...
    window.changeStatus = async function(id, status) {
        ...
        const result = await Swal.fire({ title: `Pengesahan ${confirmText}`, text: `Anda pasti mahu menukar status dokumen ini kepada ${status}?`, icon: status === 'approved' ? 'question' : 'warning', showCancelButton: true, confirmButtonColor: status === 'approved' ? '#10b981' : '#ef4444', confirmButtonText: `Ya, ${confirmText}!` });
    ...
    document.getElementById('btnTutupModal').onclick = () => viewModal.classList.add('hidden');
    filterStatus.addEventListener('change', () => { loadData(1); });
    searchInput.addEventListener('input', () => { const term = searchInput.value.toLowerCase(); Array.from(tbody.rows).forEach(row => { row.style.display = row.textContent.toLowerCase().includes(term) ? '' : 'none'; }); });
