...
<style>
    ...
    { 
        ...
        backdrop-filter: blur(10px); box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.05); 
...
<script>
    $(document).ready(function() 
    {
        ...
        function loadFaq(id) {
            ...
            $.get(`<?= base_url('faq/ajax') ?>/${id}`, function(res) {
                if(res.success && res.faqs.length > 0) {
                    ...
                    $('#faqList').html('<div class="glass-card p-20 text-center text-slate-400 font-bold">Tiada FAQ dijumpai.</div>');
                }
            });
        }
        ...
        // KEMASKINI FAQ (FIXED STYLE KAU)
        window.openEditModal = function(id, question, answer) {
            const idServis = $('#dropdownServis').val(); // AMBIL ID SERVIS SEKARANG
            Swal.fire({
                ...
                preConfirm: () => {
                    ...
                    // WAJIB HANTAR idservis SEKALI UNTUK LEPAS VALIDATION CONTROLLER
                    return { id: id, question: q, answer: a, idservis: idServis }; 
                }
            }).then((result) => {
                if (result.isConfirmed) {
                    // Gunakan style $.post macam kau, tapi kita hantar data lengkap
                    $.post(`<?= base_url('faq/update') ?>/${result.value.id}`, result.value, function() {
                        Swal.fire({ 
                            title: 'Berjaya!', 
                            text: 'FAQ telah dikemaskini.',
                            icon: 'success', 
        ...
        window.deleteFaq = function(id) {
            Swal.fire({
                title: 'Padam FAQ?',
                text: "Data akan dibuang!", icon: 'warning',
                showCancelButton: true,
                showCloseButton: true,
